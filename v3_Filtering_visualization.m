...
%% serial port 
try
    % Auto-detect and clear existing COM3 connections
...
calcWindow = 125;              % 5-second window for BPM calculation
irBuffer = zeros(1, windowSize);
t = (1:windowSize) / fs;
smoothBPM = 0;
calcCounter = 0;               % Counter for frequency-limited calculation
...
%% Audio Setup
audioFiles = {'zen.wav', 'resting.wav', 'stress.wav'}; 
audioData = cell(1,3);
fsAudio = 44100; 
for i = 1:3
    try
...
%% UI
fig = figure('Name', 'ExPulse - High Performance Biofeedback', 'Color', 'w', 'Position', [100, 100, 900, 650]);
...
while ishandle(fig)
    ...
    if ~isnan(parts(1))
        ...
        if smoothBPM > 40
            ...
            if elapsed < (beatDuration * 0.3)
                scale = 1 + 0.4 * sin((elapsed / (beatDuration * 0.3)) * pi);
                set(hHeart, 'XData', heartX * scale, 'YData', heartY * scale, 'FaceAlpha', 0.8);
            % Relaxation Phase (Diastole)
            else
                set(hHeart, 'XData', heartX, 'YData', heartY, 'FaceAlpha', 0.2);
            end
        end
