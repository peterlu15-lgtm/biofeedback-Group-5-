...
%% ── Sun (dynamic — dims with BPM) ────────────────────────────────────────
SX = W*0.86;  SY = H*0.13;
hSunHalos = gobjects(4,1);
for hi = 1:4
    r = 34 + hi*22;
    hSunHalos(hi) = fill(ax, SX+r*cos(th), SY+r*sin(th), ...
        [1 0.97 0.82], 'EdgeColor','none', 'FaceAlpha', max(0.10-hi*0.02,0));
end
hSunDisc = fill(ax, SX+32*cos(th), SY+32*sin(th), ...
...
while ishandle(fig)
    ...
    for hi = 1:4
        set(hSunHalos(hi),'FaceAlpha', max(0,(0.10-hi*0.02)*sunGlow));
    end
